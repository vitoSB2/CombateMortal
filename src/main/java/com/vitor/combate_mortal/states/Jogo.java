...
public class Jogo implements StateMethods {
	...
	private void checkCollisions() {
		...
		if (ended) {
			...
			if (endNow - endStart >= 3000) {
				...
				if (vencedor == 1)
					gp.vitoria.setVencedor(p1.personagem, 0);
				else if (vencedor == 2) {
	...
	public void draw(Graphics g) {
		...
		if (ended)
			if (vencedor == 1)
				g.drawImage(gp.vitoria.mensagem[p1.personagem], 357, 200,
						gp.vitoria.mensagem[p1.personagem].getWidth() * 3,
						gp.vitoria.mensagem[p1.personagem].getHeight() * 3, gp);
			else if (vencedor == 2)
				g.drawImage(gp.vitoria.mensagem[p2.personagem], 357, 200,
						gp.vitoria.mensagem[p2.personagem].getWidth() * 3,
						gp.vitoria.mensagem[p2.personagem].getHeight() * 3, gp);

		// HUD DE VIDA E NOME DOS LUTADORES
		// BARRAS DE VIDA
		g.drawImage(lifeBar, 50, 80, lifeBar.getWidth() * 3, lifeBar.getHeight() * 3, gp);
		g.drawImage(lifeBar, gp.getWidth() - lifeBar.getWidth() * 3 - 50, 80, lifeBar.getWidth() * 3,
				lifeBar.getHeight() * 3, gp);
		// PARTE VERDE (REPRESENTA A QUANTIDADE DE VIDA
		g.setColor(new Color(0, 180, 0));
		g.fillRect(59, 89, (486 * p1.vida) / p1.maxVida, lifeBar.getHeight() * 3 - 18);
		g.fillRect(gp.getWidth() - 59 - (486 * p2.vida) / p2.maxVida, 89, (486 * p2.vida) / p1.maxVida, lifeBar.getHeight() * 3 - 18);
		...
		g.drawImage(nomes_dir[p2.personagem], gp.getWidth() - nomes_dir[0].getWidth() * 3 - 100, 91,
				nomes_dir[0].getWidth() * 3, nomes_dir[0].getHeight() * 3, gp);

		// BARRAS DE ESPECIAL
		g.drawImage(specialBar, 50, 150, specialBar.getWidth() * 3, specialBar.getHeight() * 3, gp);
		g.drawImage(specialBar, gp.getWidth() - specialBar.getWidth() * 3 - 50, 150, specialBar.getWidth() * 3,
				specialBar.getHeight() * 3, gp);
		// PARTE AZUL
		...
		if ((p2.specialNow - p2.specialStart) < p2.specialCooldown) {
			...
			g.fillRect(
					(gp.getWidth() - specialBar.getWidth() * 3 - 44) + (((specialBar.getWidth() * 3) - 12) - widthEsp),
					156, widthEsp, specialBar.getHeight() * 3 - 18);
		} else
			g.fillRect(gp.getWidth() - specialBar.getWidth() * 3 - 44, 156, (specialBar.getWidth() * 3) - 12,
					specialBar.getHeight() * 3 - 18);
